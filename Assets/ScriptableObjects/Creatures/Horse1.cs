...
public class Horse1 : MonoBehaviour
{
    ...
    void Skill()
    {
        ...
        {
            case "item_phase":
                friends = GameObject.FindGameObjectsWithTag(this.tag);
                break;

            case "battle_phase":
                GameObject[] new_friends = GameObject.FindGameObjectsWithTag(this.tag);
                int diff = new_friends.Length - friends.Length;

                for (int i = 0; i < diff; i++)
                {
                    BasicCreatureScript script = new_friends[new_friends.Length - i - 1].GetComponent<BasicCreatureScript>();

                    script.AddAndGetAtk(3 * script.GetOrSetLevel());
                    script.AddAndGetSpecial(3 * script.GetOrSetLevel());
                }
