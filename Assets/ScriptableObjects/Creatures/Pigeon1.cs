...
public class Pigeon1 : MonoBehaviour
{
    ...
    void Skill()
    {
        ...
        foreach (GameObject creature in team)
        {
            if (creature != null)
            {
                creature.GetComponent<BasicCreatureScript>().AddAndGetMaxHp(this_creature.GetOrSetLevel());
            }
        }
    }
}